...
class UDPListPage extends StatefulWidget {
  UDPListPage({Key key, this.title}) : super(key: key);

  final String title;

  @override
  _UDPListPageState createState() => _UDPListPageState();
}

class _UDPListPageState extends State<UDPListPage> {
  final _biggerFont = const TextStyle(fontSize: 18.0);
  List<SessionConfig> _SessionList = [];
  List<UDPConfig> _UDPList = [];
  ...
  Future _addUDP(SessionConfig config) async {
                ...
                actions: <Widget>[
                  ...
                  new FlatButton(
                    ...
                    onPressed: () {
                      ...
                      udpConfig.runId = config.runId;
                      udpConfig.description = _description_controller.text;
                      udpConfig.localIP = _local_ip_controller.text;
                      udpConfig.localProt =
                          int.parse(_local_port_controller.text);
                      udpConfig.remoteIP = _remote_ip_controller.text;
                      udpConfig.remotePort =
                          int.parse(_remote_port_controller.text);
                      createOneUDP(udpConfig).then((restlt) {
//                                :TODO 添加内网之后刷新列表
                        Navigator.of(context).pop();
                      });
                    },
  ...
  void _pushDetail(UDPConfig config) async {
                          ...
                          builder: (_) => new AlertDialog(
                                  ...
                                  actions: <Widget>[
                                    ...
                                    new FlatButton(
                                      child: new Text("删除"),
                                      onPressed: () {
                                        deleteOneUDP(config).then((result) {
                                          Navigator.of(context).pop();
                                        });
//                                  ：TODO 删除之后刷新列表
                                      },
