...
import 'package:openiothub/pages/common/web/simple_embedded_browser.dart';
...
class _InstalledAppsPageState extends State<InstalledAppsPage> {
  ...
  void initState() {
    ...
    _getCsrfToken().then((_) => _initListTiles());
  ...
  Future<void> _initListTiles() async {
    ...
    final response = await dio.getUri(Uri.parse(reqUri));
    var dockerContainerList = _getDockerContainerList(response.data);
  ...
  _removeApp(String containerId, appName) async {
    ...
    // TODO
    // action=remove_container&container=0fb9fdd30fef&name=ddns-go&csrfToken=E5FDD8E7277F5CC6
  ...
  _changeAppStatus(String containerId, status) async {
    ...
    String reqUri = "/plugins/dynamix.docker.manager/include/Events.php";
    //Form Data action=stop&container=0fb9fdd30fef&csrfToken=E5FDD8E7277F5CC6
    final response = await dio.postUri(Uri.parse(reqUri),
        data: FormData.fromMap({
          "action": status,
          "container": containerId,
  ...
  _openWithWebBrowser(String ip, int port) async {
    if (!Platform.isAndroid) {
      ...
      Navigator.push(context, MaterialPageRoute(builder: (ctx) {
        final u = "http://$ip:$port";
        return SimpleEmbeddedBrowserScaffold(
          initialUrl: u,
          title: OpenIoTHubLocalizations.of(ctx).web_browser,
          onOpenInSystemBrowser: () => _launchUrl(u),
        );
      }));
    }
  }

  List<Map<String, dynamic>> _getDockerContainerList(String htmlDocument) {
    List<Map<String, dynamic>> dockerContainerList = [];
