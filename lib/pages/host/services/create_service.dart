...
class CreateServiceWidgetState extends State<CreateServiceWidget> {
  ...
  String? _selectedApplicationOption;
  // TODO 添加UDP、FTP端口
  TextEditingController nameController = TextEditingController.fromValue(
    TextEditingValue(text: ""),
  );
  TextEditingController descriptionController = TextEditingController.fromValue(
    TextEditingValue(text: ""),
  );
  TextEditingController remotePortController = TextEditingController.fromValue(
    const TextEditingValue(text: "80"),
  );
  TextEditingController localPortController = TextEditingController.fromValue(
  ...
  Widget build(BuildContext context) {
    if (_selectedNetworkOption == null || _selectedNetworkOption!.isEmpty) {
      setState(() {
        _selectedNetworkOption = _networkProtocols.first;
      });
    }
    if (_selectedApplicationOption == null ||
        _selectedApplicationOption!.isEmpty) {
      setState(() {
        _selectedApplicationOption = _applicationProtocols.first;
      });
    }
    ...
    if (descriptionController.text.isEmpty) {
      setState(() {
        descriptionController.text =
            OpenIoTHubLocalizations.of(context).my_tcp_port;
      });
    }
    return AlertDialog(
        ...
        child: ListView(
          children: <Widget>[
            ...
            DropdownButton<String>(
              value: _selectedNetworkOption, // 当前选中的值
              ...
              onChanged: (String? newValue) {
                // 当选项改变时调用此函数
                setState(() {
                  // 使用setState来更新状态，从而触发界面重新构建
                  _selectedNetworkOption = newValue; // 更新当前选中的值
                });
              },
              items:
                  _networkProtocols // 下拉菜单的选项列表
                      .map<DropdownMenuItem<String>>((String value) {
                        // 将字符串列表转换为DropdownMenuItem列表
            ...
            DropdownButton<String>(
              value: _selectedApplicationOption, // 当前选中的值
              icon: Icon(Icons.arrow_downward), // 下拉图标，可以自定义
              iconSize: 18, // 图标大小，可以自定义
              elevation: 16, // 下拉阴影高度，可以自定义
              style: TextStyle(color: Colors.blue), // 下拉菜单样式，可以自定义文本样式等
              ...
              onChanged: (String? newValue) {
                ...
                setState(() {
                  // 使用setState来更新状态，从而触发界面重新构建
                  _selectedApplicationOption = newValue; // 更新当前选中的值
                });
              },
              items:
                  _applicationProtocols // 下拉菜单的选项列表
                      .map<DropdownMenuItem<String>>((String value) {
                        // 将字符串列表转换为DropdownMenuItem列表
                        return DropdownMenuItem<String>(
                          value: value, // 选项的值，用于与_selectedOption比较和赋值
      ...
      actions: <Widget>[
        ...
        TextButton(
          ...
          onPressed: () async {
            ...
            try {
              tcpConfig.remotePort = int.parse(remotePortController.text);
              tcpConfig.localProt = int.parse(localPortController.text);
            } catch (e) {
              showFailed(
